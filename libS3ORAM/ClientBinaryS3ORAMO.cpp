...
unsigned long int ClientBinaryS3ORAMO::exp_logs[9];
unsigned long int ClientBinaryS3ORAMO::thread_max = 0;
char ClientBinaryS3ORAMO::timestamp[16];



zmq::context_t** ClientBinaryS3ORAMO::context = new zmq::context_t*[NUM_SERVERS];
zmq::socket_t**  ClientBinaryS3ORAMO::socket = new zmq::socket_t*[NUM_SERVERS];
...
ClientBinaryS3ORAMO::ClientBinaryS3ORAMO()
{
    ...
    for (TYPE_INDEX i = 0 ; i < NUM_SERVERS ; i++)
    {
		...
		auto end = time_now;
		cout<< "	[ClientBinaryS3ORAMO] " << 2*PRECOMP_SIZE << " Logical Values Precomputed in" << std::chrono::duration_cast<std::chrono::nanoseconds>(end-start).count()<< " ns"<<endl;
...
int ClientBinaryS3ORAMO::saveState()
{
 ...
 * @return 0 if successful
 */ 
int ClientBinaryS3ORAMO::loadState()
{
	FILE* local_data = NULL;
 ...
 * @return 0 if successful
 */  
int ClientBinaryS3ORAMO::sendORAMTree()
{
 ...
 * @return 0 if successful
 */  
int ClientBinaryS3ORAMO::getLogicalVector(TYPE_DATA* logicalVector, TYPE_ID blockID)
{
 ...
 * @return 0 if successful
 */  
int ClientBinaryS3ORAMO::getEvictMatrix(TYPE_DATA** evictMatrix, TYPE_INDEX n_evict)
{
	S3ORAM ORAM;
 ...
 * @return 0 if successful
 */  
void* ClientBinaryS3ORAMO::thread_socket_func(void* args)
{
...
int ClientBinaryS3ORAMO::sendNrecv(int peer_idx, unsigned char* data_out, size_t data_out_size, unsigned char* data_in, size_t data_in_size, int CMD)
{	
    unsigned char buffer_in[sizeof(CMD_SUCCESS)];
	unsigned char buffer_out[sizeof(CMD)];
