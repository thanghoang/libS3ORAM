...
zmq::context_t** ServerKaryS3ORAMO::context_send = new zmq::context_t*[NUM_SERVERS-1];
zmq::socket_t**  ServerKaryS3ORAMO::socket_send = new zmq::socket_t*[NUM_SERVERS-1];
    

zmq::context_t** ServerKaryS3ORAMO::context_recv = new zmq::context_t*[NUM_SERVERS-1];
zmq::socket_t** ServerKaryS3ORAMO::socket_recv = new zmq::socket_t*[NUM_SERVERS-1];



unsigned long int ServerKaryS3ORAMO::server_logs[13];
unsigned long int ServerKaryS3ORAMO::thread_max = 0;
char ServerKaryS3ORAMO::timestamp[16];

ServerKaryS3ORAMO::ServerKaryS3ORAMO(TYPE_INDEX serverNo, int selectedThreads) 
{
...
int ServerKaryS3ORAMO::recvORAMTree(zmq::socket_t& socket)
{
    int ret = 1;
...
int ServerKaryS3ORAMO::retrieve(zmq::socket_t& socket)
{
    ...
    for(int i = 0, startIdx = 0; i < numThreads , startIdx < DATA_CHUNKS; i ++, startIdx+=step)
    {
        ...
        pthread_create(&thread_compute[i], NULL, &ServerKaryS3ORAMO::thread_loadRetrievalData_func, (void*)&loadData_args[i]);
    ...
    for(int i = 0, startIdx = 0 ; i < numThreads , startIdx < DATA_CHUNKS; i ++, startIdx+=step)
    {
        ...
        pthread_create(&thread_compute[i], NULL, &ServerKaryS3ORAMO::thread_dotProduct_func, (void*)&dotProduct_args[i]);
 ...
 * @return 0 if successful
 */  
int ServerKaryS3ORAMO::recvBlock(zmq::socket_t& socket)
{
...
int ServerKaryS3ORAMO::evict(zmq::socket_t& socket)
{
    ...
    for(int h = 0; h < H+1 ; h++)
    {
		...
		for(int i = 0, startIdx = 0; i < numThreads , startIdx < DATA_CHUNKS; i ++, startIdx+=step)
		{
            ...
            pthread_create(&thread_compute[i], NULL, &ServerKaryS3ORAMO::thread_loadBucket_func, (void*)&loadData_args[i]);
		...
		for(TYPE_INDEX k = 0; k < NUM_SERVERS-1; k++)
		{
			...
			pthread_create(&thread_recv[k], NULL, &ServerKaryS3ORAMO::thread_socket_func, (void*)&recvSocket_args[k]);
		}
		...
		for (int i = 0; i < NUM_SERVERS-1; i++)
		{
			...
			pthread_create(&thread_send[i], NULL, &ServerKaryS3ORAMO::thread_socket_func, (void*)&sendSocket_args[i]);
		}
		cout<< "	[evict] CREATED!" <<endl;
		//=================================================================================================================
...
int ServerKaryS3ORAMO::multEvictTriplet(int level)
{
    ...
    for(int i = 0, startIdx = 0 ; i < numThreads; i ++, startIdx+=step)
    {
        ...
        if(level == H)
        {
            ...
            pthread_create(&thread_compute[i], NULL, &ServerKaryS3ORAMO::thread_crossProduct_func, (void*)&crossProduct_args[i]);
        }
        else
        {
 ...
 * @return 0 if successful
 */  
void *ServerKaryS3ORAMO::thread_socket_func(void* args)
{
...
int ServerKaryS3ORAMO::send(int peer_idx, unsigned char* input, size_t inputSize)
{
...
int ServerKaryS3ORAMO::recv(int peer_idx, unsigned char* output, size_t outputSize)
{
    try
    {
 ...
 * 
 */  
void *ServerKaryS3ORAMO::thread_dotProduct_func(void* args)
{
 ...
 * 
 */  
void *ServerKaryS3ORAMO::thread_crossProduct_func(void* args)
{
    THREAD_COMPUTATION* opt = (THREAD_COMPUTATION*) args;
 ...
 * 
 */  
void* ServerKaryS3ORAMO::thread_loadRetrievalData_func(void* args)
{
 ...
 * 
 */  
void* ServerKaryS3ORAMO::thread_loadBucket_func(void* args)
{
    THREAD_LOADDATA* opt = (THREAD_LOADDATA*) args;
